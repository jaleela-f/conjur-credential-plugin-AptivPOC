...
public class JwtToken {
	...
	public String sign() {
		...
		try {
			...
			// LOGGER.log(Level.FINEST, "Return: " +
	...
	protected static JwtRsaDigitalSignatureKey getCurrentSigningKey(JwtToken jwtToken) {
		...
		while (iterator.hasNext()) {
			...
			// LOGGER.log(Level.FINE, "currentTime: " + currentTime + " creationTime: " +
			// key.getCreationTime() + " max_key_time_in_sec: " + max_key_time_in_sec + "
			// exp: " + jwtToken.claim.getLong("exp"));
	...
	protected static JSONObject getJwkset() {
		...
		//long max_key_time_in_sec = GlobalConjurConfiguration.get().getKeyLifetimeInMinutes() * 60;
		Iterator<JwtRsaDigitalSignatureKey> iterator = keysQueue.iterator();
		LOGGER.log(Level.FINE, "KeyOueue size" + keysQueue.size());
		...
		if (keysQueue.size()>0) {
			LOGGER.log(Level.FINE, "Public key to write " + jwks.toString());
			JWTTokenStorage.writeToFile(jwks.toString());
		}
		return jwks;
